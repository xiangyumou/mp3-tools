...
test.describe('Wizard Flow - Full E2E Tests', () => {
    test.beforeEach(async ({ page }) => {
        ...
        await expect(page.locator('text=What would you like to do?')).toBeVisible({ timeout: 60000 });
    });

    test.describe('Step 1: Mode Selection', () => {
        test('displays all three mode options', async ({ page }) => {
            await expect(page.locator('text=Concatenate Intro/Outro')).toBeVisible();
            await expect(page.locator('text=Trim Audio')).toBeVisible();
            await expect(page.locator('text=Trim & Concatenate')).toBeVisible();
        });
        ...
        test('selecting a mode enables the next button', async ({ page }) => {
            ...
            await page.locator('text=Concatenate Intro/Outro').click();
        ...
        test('selected mode is visually highlighted', async ({ page }) => {
            const concatCard = page.locator('button:has-text("Concatenate Intro/Outro")');
    ...
    test.describe('Step 2: Configuration', () => {
        test('shows concat settings for concat mode', async ({ page }) => {
            ...
            await expect(page.locator('text=Configure your settings')).toBeVisible();
            await expect(page.locator('text=Concatenation Settings')).toBeVisible();
            await expect(page.locator('text=Intro File (Optional)')).toBeVisible();
            await expect(page.locator('text=Outro File (Optional)')).toBeVisible();
        });
        ...
        test('shows trim settings for trim mode', async ({ page }) => {
            ...
            await expect(page.locator('text=Start Time (seconds)')).toBeVisible();
            await expect(page.locator('text=Duration (seconds)')).toBeVisible();
        });

        test('shows both settings for both mode', async ({ page }) => {
            // Select both mode
            await page.locator('text=Trim & Concatenate').click();
            await page.locator('button:has-text("Next")').click();

            // Should show both settings
            await expect(page.locator('text=Concatenation Settings')).toBeVisible();
            await expect(page.locator('text=Trim Settings')).toBeVisible();
        });
    ...
    test.describe('Step 3: File Selection', () => {
        test('navigates to file selection step', async ({ page }) => {
            ...
            // Should be on step 3
            await expect(page.locator('text=Select your audio files')).toBeVisible();
            await expect(page.locator('text=Click to select or drag files here')).toBeVisible();
        });
    ...
    test.describe('Navigation Flow', () => {
        test('step indicator shows current step', async ({ page }) => {
            ...
            const step1 = page.locator('text=Select Mode').first();
        ...
        test('step indicator updates on navigation', async ({ page }) => {
            ...
            const step2 = page.locator('text=Configure');
    ...
    test.describe('Responsive Layout', () => {
        test('mode cards stack on mobile', async ({ page }) => {
            ...
            // Wait for content
            await expect(page.locator('text=What would you like to do?')).toBeVisible({ timeout: 30000 });
